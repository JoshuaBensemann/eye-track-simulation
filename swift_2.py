...
def nonlabile_step(time_stamp, current_activations):
    target_word, nonlabile_timer = start_nonlabile_stage(current_activations)
    nonlabile_timer = nonlabile_timer + time_stamp

    return target_word, nonlabile_timer


def saccade_step(time_stamp, word_pos, k, target_word):
    target_pos = word_pos[target_word, :]
    post_saccade_k, execution_timer, suppression_delay = start_saccade_execution(k, target_pos)
    suppression_start = time_stamp + suppression_delay
    suppression_stop = time_stamp + execution_timer + suppression_delay

    return post_saccade_k, execution_timer, target_pos, suppression_start, suppression_stop
...
def run(trial_id, df):
    ...
    for i in range(1, simulation_steps):
        ...
        if nonlabile_timer == 0:  # at point of no return if > 0
            activation_integral = calculate_activation_integral(trial[:i + 1, meta_data[i, 3]], time_stamp, time_step)
            ...
            if time_stamp <= labile_timer:
                target_word, nonlabile_timer = nonlabile_step(time_stamp, trial[i, :])

        else:
            if time_stamp <= nonlabile_timer and execution_timer == 0:
                post_saccade_k, execution_timer, target_pos, suppression_start, suppression_stop = \
                    saccade_step(time_stamp, word_pos, k, target_word)
